...
doesn't have references so we return a new cursor in a {"node": …,
"next": …} pair (same pattern as the lexer's scanners).
Aliases for typed map access. orion-self treats get_int/get_map as
builtins that return i64/Map respectively; lodge-orion needs an .or
definition. Both call into plain `get`.
